...
if ( empty( $books ) ) {
	echo '<div ' . $wrapper_attributes . '><p class="seer-reading-list__empty">' . esc_html__( 'No books found.', 'seer' ) . '</p></div>';
	return;
}
...
foreach ( $books as $book ) {
	...
	$GLOBALS['seer_current_book'] = $book;
...
unset( $GLOBALS['seer_current_book'] );

$html = '<ul class="seer-reading-list__grid">' . $items . '</ul>';

if ( $pagination_block ) {
	$GLOBALS['seer_current_pagination'] = array(
		'page'     => $page,
		'total'    => $total_pages,
		'page_arg' => $page_arg,
	);
	...
	unset( $GLOBALS['seer_current_pagination'] );
}
